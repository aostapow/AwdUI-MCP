...
AwdUI resolves UI elements through **layers**, each available if the previous fails.
...
- `~/.awdui-mcp/repositories/{app_id}.json` — object definitions
- `~/.awdui-mcp/repositories/{app_id}/assets/` — crop, template, annotated images
- `~/.awdui-mcp/strategy_memory.json` — winning strategies per object

## Sidecars

- `awdui-uia-sidecar` — FlaUI UIA3 interaction
- `awdui-spy-sidecar` — Spy-grade inspect + highlight overlay

Build: `mcp-servers\awdui-spy-sidecar\build.cmd`
