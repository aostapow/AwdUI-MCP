...
How AwdUI works with different UI toolkits on Windows.
...
**Note:** AwdUI distinguishes browsers from Electron apps automatically by process name. Both use the `Chrome_WidgetWin_1` window class, but browser processes (chrome.exe, msedge.exe, etc.) are classified as `chromium_browser` while Electron apps (Code.exe, Slack.exe, etc.) are classified as `electron`.
