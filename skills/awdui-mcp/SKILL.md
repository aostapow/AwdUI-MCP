...
# AwdUI — Screen Automation Workflow

## Overview

You have eyes and hands. Use the AwdUI MCP tools to see the screen, click, type, scroll, and interact with any application. This skill teaches you how to use them effectively.

## Browser Tasks — Playwright Bridge

If Playwright MCP tools are available (`mcp__plugin_playwright_playwright__browser_*`), use them for all in-browser interactions. AwdUI handles everything outside the browser.
...
- **AwdUI:** non-browser GUI apps, native dialogs (file pickers, download prompts, auth popups), and visual verification of what the user actually sees on screen

### The Desync Problem

By default, Playwright launches its own browser instance. This means Claude and the user are looking at **different browsers** — Playwright sees its browser, the user sees theirs. Visual verification with AwdUI will show the user's screen, not what Playwright is working on.
...
**Fallback:** If the token isn't set and the prompt appears, use AwdUI `click_text("Allow")` to approve — but this only works if the Playwright call hasn't already timed out.

**Once bridged, the interaction loop is:**

1. **AwdUI `screenshot`** — see what the user actually sees on screen
...
4. **AwdUI `screenshot`** — verify the result on the user's actual screen

This gives you DOM precision (Playwright) with visual ground truth (AwdUI).

### Fallback

**Always fall back to AwdUI for browser work if:**
...
Don't retry Playwright if it fails — switch to AwdUI immediately and use the Web Form Pattern below. The bridging setup is optional; AwdUI works fine for browser tasks on its own.

### Quick Check

To detect if bridging is active, try `browser_snapshot` — if it shows the same page the user has open (verify with a AwdUI screenshot), you're bridged. If it shows a different page or blank tab, Playwright is running its own browser.
...
> **macOS:** Ensure your terminal has Accessibility permissions (System Settings > Privacy & Security > Accessibility). AwdUI checks this on startup and warns if not granted.

## Hands On / Hands Off

**Always announce intent and show a visible status when interacting with the screen.**

Before your first AwdUI tool call in a sequence:
...
> **macOS shortcuts:** On macOS, use `cmd` instead of `ctrl` for most shortcuts. For example: `cmd+t` (new tab), `cmd+w` (close tab), `cmd+l` (address bar), `cmd+s` (save), `cmd+a` then `cmd+c` (select all + copy), `cmd+v` (paste). Use `option` instead of `alt` (e.g., `option+left` for navigate back). AwdUI automatically detects the platform.
...
4. `smart_find` or `discover_target_tool` — full loop with trace in `~/.awdui-mcp/traces/`
...
You maintain a persistent playbook at `~/.claude/awdui-mcp/playbook.md` that accumulates patterns learned from UI automation sessions. This helps you avoid repeating mistakes and get better over time.

### Session Start
At the beginning of any AwdUI session, check if the playbook exists:
1. Read `~/.claude/awdui-mcp/playbook.md` using the Read tool
