"""AwdUI MCP Server -- screen capture and interaction for Claude."""
...
# cache directory.  Use ~/.awdui-mcp/.venv as a stable, user-local location.
_AWDUI_DATA = os.path.join(os.path.expanduser("~"), ".awdui-mcp")
_VENV_DIR = os.path.join(_AWDUI_DATA, ".venv")
...
if not os.path.exists(_VENV_SITE):
    sys.stderr.write("[AwdUI] Creating virtual environment...\n")
    subprocess.check_call(
        [sys.executable, "-m", "venv", _VENV_DIR],
        stdout=subprocess.DEVNULL,
    )
    _req = os.path.join(_SERVER_DIR, "requirements.txt")
    sys.stderr.write("[AwdUI] Installing dependencies...\n")
    subprocess.check_call(
    ...
    if sys.platform == "darwin":
        sys.stderr.write("[AwdUI] Installing macOS dependencies (PyObjC)...\n")
        subprocess.check_call(
            [_VENV_PIP, "install", "-q",
             "pyobjc-core",
        ...
        sys.stderr.write("[AwdUI] Installing Windows dependencies...\n")
        subprocess.check_call(
            [_VENV_PIP, "install", "-q", "pywinauto", "pyvda", "comtypes"],
            stdout=subprocess.DEVNULL,
        )
    sys.stderr.write("[AwdUI] Ready.\n")
...
def _macos_preflight_permissions():
    ...
    if missing:
        ...
        sys.stderr.write(
            f"[AwdUI] macOS permissions needed: {perms}\n"
            f"[AwdUI] Grant access in System Settings > Privacy & Security,\n"
            f"[AwdUI] then restart the server for changes to take effect.\n"
        )
    else:
        sys.stderr.write("[AwdUI] macOS permissions OK.\n")
...
SCREENSHOT_DIR = os.path.join(tempfile.gettempdir(), "awdui_screenshots")
screenshot_mgr = ScreenshotManager(SCREENSHOT_DIR)

mcp = FastMCP("awdui-mcp")

# Register all tool modules
from tools import screenshot, input_tools, windows, manage, uac, desktop, ui_automation, ocr, batch, framework_detect, automation_profile, target_window, visual_diff, watcher, version, discovery, wait_tools, session_tools, form_tools, element_read_tools, event_monitor, ascii_view
...
automation_profile.register(mcp)
