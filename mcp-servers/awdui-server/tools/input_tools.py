...
pyautogui.PAUSE = 0.01      # tightened — batch/fast mode relies on minimal delay
...
def do_click(x: int, y: int, button: str = "left", clicks: int = 1, *, verify_visual: bool | None = None) -> dict:
    ...
    if verify_visual is None:
        verify_visual = verify_visual_changes()
    ...
    result["visual_change"] = pixel_diff >= 0.005
    result["pixel_diff"] = pixel_diff
...
def do_scroll(
    ...
    result = {
        ...
        "y": y,
        "direction": direction,
        "scroll_detected": pixel_diff >= 0.005 if verify else None,
        "pixel_diff": pixel_diff if verify else None,
    }
    if use_keyboard:
    ...
    if verify and pixel_diff < 0.005:
...
def register(server) -> int:
    ...
    def click(
        ...
        msg = f"Clicked ({result['button']}) at ({result['x']}, {result['y']}) x{result['clicks']}."
        if result.get("navigation_warning"):
            msg += f"\n\u26a0\ufe0f {result['navigation_warning']}"
        if result.get("visual_change") is not None:
