"""Version detection and update notification for AwdUI."""
...
GITHUB_REPO = os.environ.get("AWDUI_GITHUB_REPO", "aostapow/AwdUI-MCP")
CHECK_INTERVAL_SECONDS = int(
    os.environ.get("AWDUI_VERSION_CHECK_INTERVAL", str(24 * 3600))
)
USER_AGENT = "AwdUI-MCP"
_CACHE_FILE = os.path.join(os.path.expanduser("~"), ".awdui-mcp", "version_check.json")
...
def maybe_notify_update() -> None:
    ...
    if os.environ.get("AWDUI_SKIP_VERSION_CHECK", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    def _run() -> None:
        try:
            ...
            if info.update_available:
                sys.stderr.write(f"[AwdUI] {info.format_status()}\n")
                sys.stderr.write(
                    "[AwdUI] Update with: python scripts/update.py "
                    "(or restart MCP — launcher auto-updates when AWDUI_AUTO_UPDATE=true)\n"
                )
        except Exception:
            # Never block or crash MCP startup for version checks.
            return

    threading.Thread(target=_run, daemon=True, name="awdui-version-check").start()
