...
class DetectionOrchestrator:
    ...
    def detection_health(self, window_title: Optional[str] = None) -> dict:
        ...
        import os
        full = os.environ.get("AWDUI_HEALTH_FULL", "").strip() in ("1", "true", "yes")
        ...
        for name, backend in self._backends.items():
            ...
            count = -1 if (is_fast_mode() and not full) else 0
            if available and not (is_fast_mode() and not full):
                try:
    ...
    def smart_find(
        ...
        if (
            ...
            and remember
            and result.elements
            and (remember_snapshot or remember_snapshots())
        ):
            try:
