...
SERVER_DIR="$(cd "${SCRIPT_DIR}/../mcp-servers/awdui-server" && pwd)"
...
if [ -n "$CURRENT_CMD" ] && [ "$CURRENT_CMD" != "$PYTHON" ]; then
    ...
    echo "[AwdUI] Patched plugin.json: ${CURRENT_CMD} → ${PYTHON}" >&2
fi

# Only install if venv doesn't exist
if [ ! -d "${VENV_DIR}" ]; then
    echo "[AwdUI] Installing dependencies..." >&2
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        echo "[AwdUI] Installing macOS dependencies (PyObjC)..." >&2
        "${PIP}" install -q \
            pyobjc-core \
        ...
        echo "[AwdUI] Installing Windows dependencies..." >&2
        "${PIP}" install -q pywinauto pyvda comtypes
    fi

    echo "[AwdUI] Dependencies installed." >&2
fi
...
if missing:
    ...
    print(f'[AwdUI] macOS permissions needed: {perms}', file=sys.stderr)
    print(f'[AwdUI] Grant access in System Settings > Privacy & Security,', file=sys.stderr)
    print(f'[AwdUI] then restart for changes to take effect.', file=sys.stderr)
else:
    print('[AwdUI] macOS permissions OK.', file=sys.stderr)
" 2>&1 || true
fi
