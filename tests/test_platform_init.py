"""Tests for the awdui_platform package scaffolding.
...
# Make the awdui-server package importable
sys.path.insert(
    0,
...
class TestPlatformAutoSelect:
    ...
    def test_darwin_backend_selected_on_macos(self):
        ...
        from awdui_platform import darwin_backend

        for name in REQUIRED_NAMES:
            pkg_fn = getattr(awdui_platform, name)
            backend_fn = getattr(darwin_backend, name)
            assert pkg_fn is backend_fn, (
                f"awdui_platform.{name} should come from darwin_backend"
            )
    ...
    def test_unsupported_platform_raises(self):
        ...
        with mock.patch.object(sys, "platform", "freebsd"):
            ...
            with pytest.raises(RuntimeError, match="does not support platform"):
                importlib.reload(
                    __import__("awdui_platform")
                )

        # Restore the real module so other tests are unaffected
        importlib.reload(__import__("awdui_platform"))
...
class TestExportedNames:
    ...
    def test_function_exported(self, name):
        ...
        assert hasattr(awdui_platform, name), (
            f"awdui_platform is missing expected export: {name}"
        )
        assert callable(getattr(awdui_platform, name)), (
            f"awdui_platform.{name} should be callable"
        )
...
class TestDarwinStubFunctions:
    ...
    def test_stub_raises_not_implemented(self, name):
        ...
        fn = getattr(awdui_platform, name)
