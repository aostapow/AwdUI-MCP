"""Smoke test all AwdUI MCP tools via direct Python calls."""
...
SERVER_DIR = os.path.join(ROOT, "mcp-servers", "awdui-server")
sys.path.insert(0, SERVER_DIR)

_VENV_SITE = os.path.join(os.path.expanduser("~"), ".awdui-mcp", ".venv", "Lib", "site-packages")
...
def main() -> int:
    print("=== AwdUI Full Tools Smoke Test ===\n")
    ...
    if elements:
        ...
        safe("set_element_value", lambda: do_set_element_value(
            "AwdUI test", name="Editor de texto", window_title="Bloc de notas"
        ))
