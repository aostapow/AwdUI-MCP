...
class TestFrameworkCapabilities:
    ...
    def test_winforms_button_discourages_invoke(self):
        from detection.framework_capabilities import check_tool_capability
    ...
    def test_uwp_profile_notes(self):
        from detection.framework_capabilities import build_automation_profile

        profile = build_automation_profile(
            window_title="Sample UWP",
            app_name="SampleApp.exe",
            repo_framework="uwp",
        )
        assert profile["framework"] == "uwp"
        assert any("ApplicationFrameHost" in n for n in profile["notes"])
    ...
    def test_get_automation_profile_tool(self):
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            mp.setattr(
                "detection.app_identity.repository_app_name",
                lambda _fw, _wt: ("MyApp.exe", r"C:\Apps\MyApp.exe"),
            )
            mp.setattr(
            ...
            profile = do_get_automation_profile("My WinForms App")
        assert profile["framework"] == "winforms"
